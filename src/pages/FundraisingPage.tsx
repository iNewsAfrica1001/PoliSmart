  ...
  return (
    <div className="fundraising-page">
      <header className="ops-heading"><div><span className="eyebrow">CAMPAIGN ADMINISTRATION</span><h1>Fundraising management</h1><p>Organize fundraising activity handled outside PoliSmart. PoliSmart does not process contributions.</p></div>
        <button className="primary-action" disabled={!canManage || !campaignId} title={!canManage ? "Your role has read-only fundraising access" : !campaignId ? "Create a campaign first" : undefined} onClick={() => setShowForm((value) => !value)}><Plus /> Add record</button>
      </header>
      ...
      <label className="campaign-picker campaign-context"><span><strong>Campaign context</strong>All fundraising records are restricted to this organization and campaign.</span><select value={campaignId} onChange={(event) => setCampaignId(event.target.value)} disabled={!campaigns.length}><option value="">Create a campaign first</option>{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select></label>
      <section className="fundraising-metrics" aria-label="Fundraising overview"><article><HandCoins /><strong>{data.goals.length}</strong><span>Active goals</span></article><article><Banknote /><strong>{data.confirmedTotals.map((item) => `${item.currency} ${item.amount.toLocaleString()}`).join(" · ") || "—"}</strong><span>Confirmed external records</span></article><article><UsersRound /><strong>{data.contacts.length}</strong><span>Contacts</span></article><article><CalendarClock /><strong>{data.followUps.filter((item) => item.status === "OPEN").length}</strong><span>Open follow-ups</span></article></section>
