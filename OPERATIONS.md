...
Shared Upstash REST rate limits protect authentication, API, and AI-cost boundaries across Vercel instances. Treat local counters only as bounded outage fallback; OpenAI-backed generation must fail closed when shared enforcement is unavailable. Scale Blob/storage tier before 70% quota and set lifecycle/retention rules. Raise AI budgets only after reviewing cost per successful grounded answer, tenant/feature outliers, cache/retrieval opportunities, and business approval. Never relax safety, grounding, tenant isolation, or minimum-sample safeguards to gain throughput.
