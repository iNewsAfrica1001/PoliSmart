...
for (const failingOperation of ["authUser.create", "organization.create", "membership.create"]) {
  test(`registration safely identifies ${failingOperation} failures and preserves rollback`, async () => {
    ...
    const database = {
      ...
      $transaction: async (callback) => {
        const pending = [];
        const create = (operation, value) => async () => {
        ...
        const result = await callback({
          authUser: { create: create("authUser.create", { id: "private-user-id" }) },
          organization: {
            create: create("organization.create", { id: "private-organization-id" }),
          },
          membership: { create: create("membership.create", { id: "private-membership-id" }) },
        });
    ...
    assert.equal(logs[0].includes("sensitive database message"), false);
  });
}
